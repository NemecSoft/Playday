...
/** 用户表文件名：config.json 只给目录（settings.YunGameConfigDir），文件名固定。 */
const wchar_t* const kUserListFile = L"YunGame_UserList.json";
...
struct AppConfig {
    ...
    std::wstring userListPath;    // settings.YunGameConfigDir + 固定文件名
    std::wstring gameRoot;        // settings.defaultGameRootPath
};

/**
 * 相对路径按**客户端目录**（= exe 的上级目录，因为 yungamestart\ 与客户端同级）解析 ——
 * 与桌面端 shared/pathConfig.ts 的约定一致：config.json 里的相对路径以 appRoot（应用所在目录）为基准。
 * 绝对路径原样返回（测试机 D:、正式机 X: 写的就是绝对路径）。
 *
 * 为什么要它：部署态的 config.json 里这些字段写的是相对值（`YunGameConfig`、`YunGame/PlayNite`…），
 * 而本程序可能被开机自启拉起、工作目录是别的什么地方 —— 直接拿相对值去访问必然扑空。
 */
inline std::wstring ResolveAgainst(const std::wstring& clientDir, const std::wstring& p) {
    if (p.empty()) return p;
    const bool absolute = (p.size() >= 2 && p[1] == L':')                  // C:\...
                       || (p.size() >= 2 && p[0] == L'\\' && p[1] == L'\\');  // \\server\share
    return absolute ? p : JoinPath(clientDir, p);
}
...
AppConfig LoadConfig(const Options& o, const std::wstring& exeDir) {
    ...
    for (const auto& path : candidates) {
        ...
        cfg.loaded = true;
        std::string v;
        // 相对值一律以客户端目录（exe 的上级）为基准，见 ResolveAgainst 的说明。
        const std::wstring clientDir = ParentDir(exeDir);
        // 2026-09-17：路径表把"用户表文件"改成"配置目录"（目录内部固定两个文件名，见
        // shared/pathModes.ts 的 YunGameConfigDir）—— 所以这里只读目录，文件名内部固定。
        // 旧键 yunGameUserListPath 已随这次收口彻底删掉（config.json 里不许再有它）。
        if (JsonFindString(raw, "YunGameConfigDir", v)) {
            cfg.userListPath = JoinPath(ResolveAgainst(clientDir, NormalizeSlashes(Utf8ToWide(TrimAscii(v)))), kUserListFile);
        }
        if (JsonFindString(raw, "defaultGameRootPath", v)) {
            cfg.gameRoot = ResolveAgainst(clientDir, NormalizeSlashes(Utf8ToWide(TrimAscii(v))));
        }
...
int DoRun(const Options& o) {
    ...
    // 用户表：config.json 优先，其次 <客户端目录>\YunGameConfig\（跟着 exe 走，换盘符也不用改），
    // 最后才是原版那两个写死的老位置。
    std::vector<std::wstring> userListCandidates;
    if (!cfg.userListPath.empty()) {
        userListCandidates.push_back(cfg.userListPath);
    } else {
        userListCandidates.push_back(JoinPath(JoinPath(ParentDir(exeDir), L"YunGameConfig"), kUserListFile));
