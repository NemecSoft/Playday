...
#  能放进 CoverImages 的封面。
#
#  位置：dev-tools/cover-optimizer/（2026-09-14 从「仓库根的 optimize-covers.bat + scripts/ 下的
...
# 仓库根）时能歪打正着；搬进 dev-tools\cover-optimizer\ 之后，同一个写法会去找 tools\config.json
...
if ($Slim) {
    ...
    if (-not $Source) {
        Write-Host "用法："
        Write-Host "  生成封面：dev-tools\cover-optimizer\optimize-covers.bat -Source <源图或源目录> [-OutDir <封面目录>]"
        Write-Host "  库瘦身  ：dev-tools\cover-optimizer\optimize-covers.bat -Slim"
        exit 1
    }
