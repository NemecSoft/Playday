...
tools\cover-optimizer\optimize-covers.bat -Slim -Dir "D:\YunGame\PlayNite\CoverImages"
```
...
2. **找仓库根靠"向上找 `path-modes.json`"**，不能靠"上一级目录"：本工具 2026-09-14 从
   `scripts/` 搬到这里，原先那种 `Split-Path -Parent $PSScriptRoot` 的推导会去 `tools\config.json`
