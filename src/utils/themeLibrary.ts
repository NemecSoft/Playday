...
export const themeLibrary: ThemeEntry[] = [
  ...
  },
  {
    ...
    category: "SaaS",
    palette: {
      // 借鉴 Recordly 的浅色 SaaS 风：淡蓝灰面板 + 克制的蓝色 accent。
      // 蓝→橙→黄渐变描边由 [data-fx="recordly"] 在 global.css 实现（主题自带固定调色板）。
      background: "#F3F5F9",
      foreground: "#1B2430",
      card: "#ffffff",
      cardForeground: "#1B2430",
  ...
  {
    // "紫色流光"配色：参考 reactbits.dev 主页的暗黑紫调 + 背景流光动效。
    // 背景是深紫黑（#120F17，reactbits 主页底色）+ 多团紫色光晕缓慢流动，
    // 由 global.css 的 body.theme-reactbits + @keyframes 实现动画（不是静态渐变）。
    // 卡片/面板半透明深紫，文字淡紫白，accent 亮紫霓虹。
    id: "p-reactbits",
    name: "Purple Flow (ReactBits)",
    zh: "紫色流光",
    category: "特别版",
    gradientClass: "theme-reactbits", // 专属流光背景，CSS 用 body.theme-reactbits 分发
    palette: {
      // bgBase/bgTop 在这个 palette 下不会被使用（body 背景被 CSS 的流光动画覆盖），
      // 但为了 fallback 不刺眼，填入深紫黑底。
      background: "#120F17",
      foreground: "#EDE7F6",
      card: "rgba(30, 22, 44, 0.72)",
      cardForeground: "#EDE7F6",
      primary: "#A855F7",
      primaryForeground: "#120F17",
      secondary: "#241C36",
      secondaryForeground: "#EDE7F6",
      muted: "#241C36",
      mutedForeground: "#C4B5FD",
      border: "#4C2A8A",
      ring: "#A855F7",
      bgBase: "#120F17",
      bgTop: "#1A1330",
      bgSidebar: "rgba(18, 15, 23, 0.6)",
      bgPanel: "rgba(26, 19, 43, 0.72)",
      bgItemHover: "#2A1F42",
      bgItemActive: "#382A54",
      bgInput: "#1E1730",
      borderStrong: "#A855F7",
