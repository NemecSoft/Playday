...
exports.updateGamePlaytime = updateGamePlaytime;
exports.updateGameLastSession = updateGameLastSession;
exports.setGameFavorite = setGameFavorite;
exports.setGameHidden = setGameHidden;
exports.libraryStats = libraryStats;
...
exports.replaceEnterpriseUsers = replaceEnterpriseUsers;
exports.getGameLibraries = getGameLibraries;
exports.upsertGameLibrary = upsertGameLibrary;
exports.deleteGameLibrary = deleteGameLibrary;
...
async function openDb() {
    ...
    const dbPath = (0, paths_1.databasePath)();
...
function allRows(sql, params = {}) {
    if (!db)
        throw new Error("数据库未打开，请先调用 openDb()");
...
// ===== 游戏库 CRUD =====
function getGames() {
    const rows = allRows("SELECT * FROM games");
    return rows.map((r) => rowToGame(r));
}
function getGame(id) {
    const row = oneRow("SELECT * FROM games WHERE id = $id", { $id: id });
...
function upsertGame(game) {
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      post_exit_script=$post_exit_script, post_exit_enabled=$post_exit_enabled`, values);
    persist();
}
...
function updateGamePlaytime(id, playtime, lastPlayed) {
    if (!db)
        throw new Error("数据库未打开");
...
function updateGameLastSession(id, seconds, endedAt) {
    if (!db)
        throw new Error("数据库未打开");
    db.run("UPDATE games SET last_session_seconds=$s, last_session_ended_at=$e, modified=$m WHERE id=$id", { $id: id, $s: seconds, $e: endedAt ?? new Date().toISOString(), $m: new Date().toISOString() });
    persist();
}
function setGameFavorite(id, favorite) {
    if (!db)
        throw new Error("数据库未打开");
...
function setGameHidden(id, hidden) {
    if (!db)
        throw new Error("数据库未打开");
...
function libraryStats() {
    ...
    for (const g of games) {
        for (const p of g.platform || [])
            platformMap.set(p, (platformMap.get(p) || 0) + 1);
        for (const g2 of g.genre || [])
            genreMap.set(g2, (genreMap.get(g2) || 0) + 1);
    }
...
function upsertUser(u) {
    if (!db)
        throw new Error("数据库未打开");
    db.run(`INSERT INTO users (id, account, password_hash, name, level, kind, ip_address, created_at, deleted_at)
...
function getUserByAccount(account) {
    const row = oneRow("SELECT * FROM users WHERE account = $account AND (deleted_at IS NULL OR deleted_at = '')", {
        $account: account,
    });
...
function getUserByIp(ip) {
    const rows = allRows("SELECT * FROM users WHERE kind='enterprise' AND (deleted_at IS NULL OR deleted_at = '')");
...
function listUsers() {
    const rows = allRows("SELECT * FROM users WHERE deleted_at IS NULL OR deleted_at = ''");
    return rows.map((r) => rowToUser(r));
}
// 列出所有用户（含软删除的），管理端用来展示"可恢复"的已删用户。
function listAllUsers() {
    const rows = allRows("SELECT * FROM users");
    return rows.map((r) => rowToUser(r));
}
...
function getUserById(id) {
    const row = oneRow("SELECT * FROM users WHERE id = $id", { $id: id });
    return row ? rowToUser(row) : null;
}
...
// 返回实际写入的数量。
function replaceEnterpriseUsers(users) {
    if (!db)
        throw new Error("数据库未打开");
...
function getGameLibraries() {
    const rows = allRows("SELECT * FROM game_libraries");
    return rows.map((r) => ({
        id: String(r.id),
        name: String(r.name),
...
function upsertGameLibrary(lib) {
    if (!db)
        throw new Error("数据库未打开");
    db.run(`INSERT INTO game_libraries (id, name, path) VALUES ($id, $name, $path)
     ON CONFLICT(id) DO UPDATE SET name=$name, path=$path`, { $id: lib.id, $name: lib.name, $path: lib.path });
    persist();
}
function deleteGameLibrary(id) {
    if (!db)
        throw new Error("数据库未打开");
...
function rowToGame(r) {
    ...
    return {
        ...
        localizedNames: (() => {
            try {
                return JSON.parse(str(r.localized_names));
            }
            catch {
                return [];
            }
        ...
        actions: (() => {
            try {
                return JSON.parse(str(r.actions));
            }
            catch {
                return [];
            }
        ...
        videos: (() => {
            try {
                return JSON.parse(str(r.videos));
            }
            catch {
                return [];
            }
